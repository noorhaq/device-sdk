...
#define SPOTFLOW_BINDESC_BUILD_ID_TOTAL (SPOTFLOW_BINDESC_BUILD_ID_MOCK_HEADER_SIZE + SPOTFLOW_BINDESC_BUILD_ID_VALUE_SIZE)
...
static const uint8_t *spotflow_build_id_ptr = &bindesc_entry_spotflow_build_id[SPOTFLOW_BINDESC_BUILD_ID_MOCK_HEADER_SIZE];
...
int spotflow_build_id_get(const uint8_t **build_id, uint16_t *build_id_len)
{
    ...
    for (int i = 0; i < spotflow_build_id_len; i++) {
        if (spotflow_build_id_ptr[i] != 0) { is_all_zero = false; break; }
    }
...
void spotflow_build_id_print(void)
{
    ...
    if (spotflow_build_id_get(&id, &len) == 0) {
        printf("Spotflow Build ID: ");
        for (int i = 0; i < len; i++) printf("%02x", id[i]);
        printf("\n");
    } else {
        printf("Spotflow Build ID not patched.\n");
    }
}
