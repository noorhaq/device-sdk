...
esp_err_t spotflow_coredump_backend(void)
{
	...
	if (err != ESP_OK) {
		ESP_LOGE(TAG, "Failed to get coredump image address/size: %s",
			 esp_err_to_name(err));
		return err;
	}
	...
	ESP_LOGI(TAG, "Coredump address: 0x%08X, size: 0x%08X bytes", (unsigned int)coredump_addr,
		 (int)coredump_size);
	...
	while (coredump_info.offset < coredump_info.size) {
		...
		if (err != ESP_OK) {
			free(chunk_buffer);
			ESP_LOGE(TAG, "Failed to read coredump chunk at offset %zu.",
