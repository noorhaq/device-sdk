	...
	while (1) {
		if (atomic_load(&mqtt_connected)) {
			if (esp_mqtt_client_get_outbox_size(client) ==
			    0) // Check the mqtt buffer is empty if not
			{
				queue_msg_t msg;
				while (
				    queue_read(&msg) &&
				    atomic_load(
					&mqtt_connected)) //Check if mqtt disconnect event is not generated.
				{
					int msg_id = esp_mqtt_client_publish(
					    client, "ingest-cbor", (const char*)msg.ptr, msg.len, 1,
					    0); // Sending MQTT message the CONFIG_SPOTFLOW_CBOR_LOG_MAX_LEN provides the buffer length
					// Error check.
					if (msg_id < 0) {
						SPOTFLOW_LOG("Error %d occured while sending mqtt",
							     msg_id);
						SPOTFLOW_LOG("Retrying");
					} else {
						SPOTFLOW_LOG("Message sent successfully. Freeing "
							     "message in Queue");
						queue_free(&msg);
					}
				}
				...
				SPOTFLOW_LOG(
				    "MQTT buffer not empty some message is being processed.");
			}
		}
		vTaskDelay(
		    pdMS_TO_TICKS(50)); // 50ms interval during each recheck we can increase it.
	}
}